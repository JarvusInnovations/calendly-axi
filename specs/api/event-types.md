...
| Op | Endpoint | Body |
| ---- | ---------- | ------ |
| Create | `POST /event_types` | required: `owner` (user URI), `name`; optional: `duration`, `duration_options`, `color`, `description`, `locations[]`, `locale`, `active` |
| Update | `PATCH /event_types/{uuid}` | same fields, partial — unspecified fields untouched |
...
- **There is no delete.** `PATCH` with `active: false` deactivates; that is the strongest removal the API offers.
