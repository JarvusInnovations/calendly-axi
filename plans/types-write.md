...
- [x] `types create --one-off` produces a dated type visible in `types list --all`. *(Live-verified 2026-08-12 — the guessed `date_setting: { type: "date_range", start_date, end_date }` shape is correct; the created one-off arrived active and was deactivated after. CORRECTION, later the same day: the list-visibility half is disproven — Adhoc types never appear in `GET /event_types` at all; they fetch by id only. Recorded in `specs/api/event-types.md`.)*
