...
- [x] `doctor` on a healthy config: all ok + headroom; on each broken state: the right single check fails with its remediation, exit 1. *(Broken-state halves fixture-verified. Live run 2026-08-11: credentials/token/organization/headroom all ok (498/500 remaining — paid tier confirmed); the hooks check correctly fails under the `.ts` dev entrypoint, so the full all-ok pass awaits a built/global install.)*
